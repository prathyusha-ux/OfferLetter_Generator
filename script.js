...
function calculateSalaryBreakdown(annualCtc) {
  ...
  // Provident Fund toggle — checked here (moved up) because Special
  // Allowance below depends on it: employer PF is only subtracted when
  // the user has PF set to "Yes". If PF is "No", nothing is deducted for
  // it and Special Allowance keeps the full remaining amount.
  const pfIncluded = document.querySelector('input[name="pfRadio"]:checked')?.value === 'yes';

  // Special allowance = Basic - HRA - (employer PF share, only if PF is on)
  const specialAllowance = Math.round(basic - hra - (pfIncluded ? employerPf : 0));
  ...
  // Total deductions used for Take Home = Professional Tax + Provident Fund (employee side, if on) + TDS (if on).
  // Employer PF is intentionally excluded here — it's used only to compute
  // Special Allowance above, and is never shown or deducted from Take Home.
...
function buildPage2(values, salary) {
  ...
  return pageWrapper(`
    ...
    <p>For the purpose of leave entitlement, the leave year shall be considered as the calendar year commencing from <strong>1 January and ending on 31 December</strong> of each year.</p>
 ...
 * ----------------------------------------------------------------------- *
 * FIX HISTORY:
 * - Originally RENDER_SCALE = 1 with JPEG quality 0.7 -> blurry PDFs.
 * - Bumped to RENDER_SCALE = 3 with lossless PNG -> sharp, but the file
 *   ballooned past Multer's 20MB limit (and would have hit Gmail/Resend's
 *   own attachment ceilings too, since base64-encoding inflates size by
 *   ~37% on top of that).
 * - Settled on RENDER_SCALE = 2 with high-quality JPEG (0.92) -> still
 *   noticeably sharper than the original blurry version, but the file
 *   stays a reasonable size for email delivery.
 * ----------------------------------------------------------------------- */
...
async function handlePrintClick() {
  ...
  // before html2canvas takes its snapshot — otherwise the very first
  // export after a page load can silently render with a fallback font.
...
async function buildLetterPdfBlob() {
  ...
  // Same font-loading guard as handlePrintClick — without this, emailing
  // right after the page loads can silently use a fallback font instead
  // of Calibri/Carlito.
  if (document.fonts && document.fonts.ready) {
    await document.fonts.ready;
  }
