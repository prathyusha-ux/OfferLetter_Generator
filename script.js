...
function calculateSalaryBreakdown(annualCtc) {
  ...
  // Total deductions used for Take Home = Professional Tax + PF-Employee + PF-Employer (if on) + TDS (if on).
  const totalDeductions = professionalTax + pfDeduction + (pfIncluded ? employerPf : 0) + tdsAmount;
...
function buildSalaryTableHtml(salary, values) {
  ...
  const pfRows = salary.pfIncluded
    ? `<tr><td>PF-Employee</td><td class="num">${formatRupees(salary.pfDeduction)}</td><td class="num">${formatRupees(salary.pfDeduction * 12)}</td></tr>
